...
The general workflow within the lynxes package is to create an interpretation, create a model, and run interpretation algorithms.
...
To begin using the Interpretation to explain models, we need to create a lynxes Model.

Creating a Model
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
To ensure formatting consistency, and common functionality among various model types and interfaces,
we require that models passed to Interpretations to be wrapped in a lynxes Model object. Currently we support regression type
...
To create a lynxes model based on a local function or method, pass in the predict function to an InMemoryModel. A user can optionally pass data samples
to the examples keyword argument. This is only used to infer output types and formats. Out of the box, lynxes allows models return numpy arrays and
...
With an Interpretation and a Model, one can run all lynxes interpretation algorithms.
